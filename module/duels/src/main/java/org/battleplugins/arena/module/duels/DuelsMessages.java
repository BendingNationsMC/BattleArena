...
public final class DuelsMessages {
    ...
    public static final Message TARGET_HAS_PENDING_REQUEST = Messages.error("duel-target-has-pending-request", "<secondary>{}</secondary> already has a pending duel request.");
    ...
    public static final Message DUEL_TARGET_CANCELLED_QUIT = Messages.info("duel-target-cancelled-quit", "Your duel request to <secondary>{}</secondary> has been cancelled as they have left the server.");
    ...
    public static final Message PARTY_LEADER_REQUIRED = Messages.error("duel-party-leader-required", "Only party leaders can initiate or accept duels while in a party.");
    public static final Message TARGET_MUST_BE_PARTY_LEADER = Messages.error("duel-target-must-be-party-leader", "Challenge the party leader of <secondary>{}</secondary> to duel them.");
    ...
    public static final Message INVALID_DUEL_ROUNDS = Messages.error("duel-invalid-rounds", "Rounds must be an odd number greater than 0.");
